...
**核心設計：未發文＝`report_date IS NULL`**（不加新欄位／新表）。⚠️ **敘獎例外：哨兵用空字串 `''`**——敘獎的軟刪除哨兵已是 `register_date IS NULL`（§6），若照抄 NULL 會被全系統的 active 判斷（`IS NOT NULL`）當成已刪除；`''` 則天然通過既有查詢（同 `is_electronic` 空字串＝未歸的慣例）。敘獎無 sender 欄，結算只補 `register_date`。牽動四處，動這功能逐一檢查：

1. **陳報頁（`tab_report`）＋敘獎頁（`tab_reward`，v1.2.1）**：敘獎頁 `_applySelfServiceMode` 只反灰日期欄（無 sender 欄）、`_submit` 帶 `register_date=''`、預覽列空日期顯示橘字「未發文」。陳報頁：覆寫 `_applyInputLock` → 先 `super()`（唯讀鎖）再 `_applySelfServiceMode`（自助模式下 `rpt_date`／`rpt_sender` 反灰＋tooltip）。`_submit` 自助模式帶 `report_date=None`／`sender_id=None`；`_submitCriminal`／`_submitGeneral` 驗證在自助模式**放行「發文人員」空值**（其餘必填不變）。反灰的陳報日期框以 `specialValueText(" ")` 哨兵**顯示空白**（v1.1.11；僅不可互動狀態使用，無鍵盤路徑、不踩可編輯空白欄的雷；切回送文者模式清哨兵並還原今天；`widgets` 的「日期空值補今天」邏輯對哨兵狀態放行）。
