...
| 版本 | 摘要 |
|------|------|
| v1.2.9-v4 | **同版號重新發布（tag `v1.2.9-v4`，`lib/version.py` 仍為 1.2.9）：測試與驗收機制強化，無功能／schema／UI 變動**。①**shell 測試層改行程隔離**：`tests/test_standalone_shell.py` 每支測試都自己建一個完整 `DocumentManager`，一個行程累積約 28 個，manager 底下有東西掛在比它長壽的宿主上（`AuthManager` 單例 signal、`QApplication` 層滾輪 guard／hover filter、combo 的 completer model 與 viewport event filter），回收時機不確定，累積到一定量就 native crash（`0xC0000005`）。改由根 `conftest.py` 的 `pytest_runtest_protocol` 把 `ISOLATED_MODULES` 內每支測試各派一個子行程**序列**執行，累積量由 28 降為 1；測試檔與產品程式一行未改。實測連跑 10 輪全綠、node ID 集合一致，關閉隔離的對照組 4 輪即崩 1 次。⚠️ **是隔離不是修復**，長壽掛載未拆、`qt` 層同風險仍在。判讀規則因此改為 **native crash 一次即視為回歸**（原為「崩一次先重跑、連兩次才算」）。②新增兩層防護測試：釘住隔離清單，並以 **AST 掃描**擋「日後新增會在行程內建 manager 的 shell 測試卻漏列」（用 AST 而非字串搜尋，才不會把 `test_lazy_tab_loading` 丟給子行程的字串腳本誤判）。③**列印基準比對改環境先行**：`print_baseline.py --check` 原本算完 101 張（90 秒以上）才印環境差異，環境一漂移就看到「上百處不同」而誤判成繪圖層壞掉（實際發生過）；改為開頭先比字型檔／matplotlib／Qt／PySide6／顯示縮放，不符即停在算圖之前（實測 0.6 秒）並給三條處理路線。結束碼分家：`1`＝影像有差異、`2`＝基準或資料庫缺失、`3`＝環境漂移未比對，另加 `--allow-environment-drift` 逃生口。④文件：PITFALLS **TST-5** 與 §4 判讀規則同步改寫並附實測數據，PRINTING §4 補環境先行與結束碼對照；併記錄「合併成一條」在隔離後實測 5 輪全綠（964 passed、3 分 15 秒，兩段為 3 分 29 秒），但**正式 gate 仍刻意維持兩段**（分層可讀、shell 判讀規則獨立、合併無時間收益）。⚠️ 因版號未進位，exe 版本資訊仍顯示 1.2.9，只能靠 Release 頁與檔名區分。完整套件 pytest 924（非 shell）＋41（shell）＋9（PII gate，零 skip）。 |
| v1.2.9-v3 | **同版號重新發布（tag `v1.2.9-v3`，`lib/version.py` 仍為 1.2.9）：PACKED.zip 改四檔＋新增網路分享捷徑工具，無 schema 變動**。①`PACKED.zip` 內容改為 `Police-Document-Manager.exe`＋`Police-Entry-Manager.exe`＋空白 `dbfile.db`＋`create_shared_shortcuts.bat` **四檔同層**；兩支 exe 改用**固定英文檔名、不帶版號**，避免從 SMB 分享啟動時的中文檔名相容性問題（單獨下載的 asset 仍帶版號）。②新增 `packaging/create_shared_shortcuts.bat`（**入庫**）：管理者在分享資料夾執行一次，即自動解析 UNC 並建立「公文收發管理系統」「公文快速登錄系統」兩支中文捷徑，各機即使網路磁碟機代號不同也能共用。支援四種擺法——分享內層資料夾（捷徑建上一層）、分享根目錄（建同一層）、映射網路磁碟（`EnumNetworkDrives` 反解）、伺服器本機路徑（`Win32_Share` 反查，⚠️ 只認一般分享並排除 `C$`／`D$`／`ADMIN$` 管理共用，否則會做出一般使用者打不開的捷徑）。以「cmd 呼叫 PowerShell 讀自己執行」實作，不產生 `.ps1` 故不受 ExecutionPolicy 限制；代價是編碼與換行屬功能依賴，已由新增的 `.gitattributes`（`*.bat text eol=crlf`）釘住。③資料庫瀏覽無 PDF 圖示的主旨改存完整文字（原本先截成「前段...」再寫入，導致 tooltip 與複製內容都被截短），長度改交由欄寬省略；有圖示者維持固定字數截斷。④文件：新增 `packaging/` 目錄定位（隨 Release 出貨、非開發工具），§7 補四種擺法對照表與兩條設計註記，README 擺放段改寫。⚠️ 因版號未進位，exe 版本資訊仍顯示 1.2.9，只能靠 Release 頁與檔名區分。 |
| v1.2.9-v2 | **同版號重新發布（tag `v1.2.9-v2`，`lib/version.py` 仍為 1.2.9）：權限最後防線與驗證機制收斂，無功能／schema 變動**。①**執行時權限複核**：跨年度重置、歸檔（電子檔／紙本兩個入口）在 modal 返回後會再確認一次身分——Qt modal 有自己的巢狀事件迴圈，管理身分開著確認框時若閒置自動登出，原本仍會用舊權限往下清空全庫或改檔名；歸檔另補 DB 寫入失敗時的檔名還原（原檔名已被其他程序重建時拒絕覆寫，連續失敗以 `ExceptionGroup` 併報）。②**排序儲存**補 `_refEditable()` 並改為回傳成功／失敗，降權時直接放棄未存排序並重載 DB 順序，杜絕管理者的暫存拖拉結果被降權後的身分提交。③**罰單並行保護**：`updateTicket()`／`updateTicketFromBrowse()` 改為五欄原值 CAS 並強制傳入開窗快照（漏傳直接 `TypeError`），比照敘獎既有寫法；改不到列時分流成「已被刪除」與「已被其他電腦修改」，後者提示後重新載入最新值。④罰單排序 key 收斂為 `lib/ticket_utils.ticketSortKey()`（原本三處各組一份），列印路徑不再重複排序。⑤**驗證機制**：pytest 正式 gate 改為五層 marker 的非 shell／shell 兩段，移除與 `unittest discover` 重複執行的 819 項；新增 `pytest.ini`／`conftest.py` 固定 offscreen、basetemp、cache 與每 worker 的 `MPLCONFIGDIR`；文件契約獨立成 `tests/test_release_documentation_contract.py`。⑥**列印基準可攜化**：改用 `tools/fake_seed_data.py` 的全虛構資料建暫存 DB（刪除會誤寫正式 `dbfile.db` 的舊 seed），缺 PNG 資料夾也能 `--check`，manifest 加記 matplotlib 版本並固定 3.11.1，101 項基準已重建且同機三輪逐位元組一致。⑦PII gate 改掃工作樹／index／HEAD（原本只掃 HEAD，未提交內容掃不到）並加上不跟隨 repo 外 symlink 的防護。⑧文件：簽收單列印拆出 `PRINTING.md`，DEVELOPER 新增 §10「執行時權限複核」。⚠️ 因版號未進位，exe 版本資訊仍顯示 1.2.9，只能靠 Release 頁與檔名區分。完整套件 pytest 906（非 shell）＋41（shell）＋9（PII gate）。 |

本節只留最近三個版本（`-v2` 重發與其原版視為同一格，故此處為 v1.2.9-v4／v1.2.9-v3／v1.2.9-v2）；**v1.2.5 以前的逐版記錄全部在 [HISTORY.md](HISTORY.md)**，進版時把被擠掉的那一列搬過去。
