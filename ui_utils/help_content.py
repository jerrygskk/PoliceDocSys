...
QUICKSTART = {
    ...
    6: ("依發文日期彙整當天所有發文，整理成簽收表。",
        ...
        "刑案陳報、一般陳報或罰單登錄任一功能設為自助取號模式時，本頁上方多出「結算發文」：單一表格列出所有未發文的刑案／一般／罰單案件，包含切換模式前殘留的未發文公文。可用類型標籤與關鍵字過濾、表頭全選（僅作用於顯示中列），選送文者後補齊發文資訊並自動產生簽收表；敘獎請至「敘獎發文」頁處理。"),
