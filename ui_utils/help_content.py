...
_DATE_GUARD_HINT = (
    "當輸入的<b>發文日期</b>明顯不合理，將啟動防呆提醒，確認是否繼續輸入。")


# 「公文快速登錄系統也有這一項」的行內標記。
# ⚠️ 用產品正式名稱、不寫「簡易版」：使用者手上那支 exe 顯示的就是這個名字，
# 換個叫法會對不起來（2026-08-08 維護者裁示）。
# ⚠️ 標的以 lib/app_profile.ENTRY_PROFILE 為準（`tab_keys`／`settings_pages`／
# `system_panels`），不是憑印象標——獨立版能力增減時要回頭核對這些標記。
_simple = '<span style="color:#8e8e93;">（公文快速登錄系統亦有）</span>'


# 兩種陳報模式的說明（單一來源，陳報／敘獎／罰單三頁共用）。
# ⚠️ 模式名稱必須與設定頁 radio 的字面完全一致（使用者要照著去設定頁找選項），
# 不可自創同義詞；契約測試 test_help_content_contract 釘著這兩個詞。
# ⚠️ 一律放在各頁的灰字補充區（hint），不寫進操作步驟——步驟只講「這一欄要填
# 什麼」，模式差異是背景知識（2026-08-08 維護者裁示）。
_MODE_HINT = (
    "<b>送文者輸入模式</b>：由送文者統一登錄，登錄時即填妥發文日期與發文人員。<br>"
    "<b>發文結算模式</b>：承辦人先取文號，發文資訊由送文者於列印頁結算補齊。")
...
HELP_PAGES = {
    0: [
        ...
        ("sec", "注意事項", [
            ("ul", [
                ...
                "發文日期為橘字代表先前已發過，再次發文將<b>覆蓋</b>舊日期。",
    ...
    2: [
        ...
        ("sec", "刑案陳報重點", [
            ("ul", [
                ...
                "「案件分類」可輸入<b>通俗用語</b>搜尋（如輸入「私」帶出「私行拘禁 → 302妨害自由」），"
                "選取後帶入正式案類名稱；通俗用語可由管理者於「資料庫設定 → 案件類型」的別名欄維護。",
                "「查獲／受理日期」依案件狀態填入日期：",
            ]),
            ("ul", [
                "<b>現行犯</b>：陳報單日期",
        ...
        ("hint", "<b>登錄日期＝取得文號日；陳報日期＝實際發文日</b>，兩者分開記錄。"),
        ("sec", "送出後還能補救", [
            ("ul", [
    ...
    3: [
        ("lead", "登錄敘獎事由與敘獎人員，輸入後可在下方預覽欄位取號。" + _simple),
        ("sec", "操作流程", [
            ("ol", [
                "選<b>發文日期</b>（預設今天）與<b>發文人員</b>。",
                "輸入<b>敘獎事由</b>。",
                "輸入<b>敘獎人員</b>；可點右側候選人員名條，或從輸入候選選取，名單外姓名也可直接輸入。",
                "按<b>確認送出</b>；登錄日期由系統自動填入今天，完成後列在下方本次登錄預覽。",
            ]),
        ]),
        ("sec", "修改與刪除", [
            ("ul", [
    ...
    4: [
        ("lead", "登錄舉發罰單，一張罰單對應一位開立人員。" + _simple),
        ("sec", "操作流程", [
            ("ol", [
                "選<b>發文人員</b>。",
        ...
        ("sec", "編號規則", [
            ("p", "罰單編號僅接受半形英文字母與數字，系統會自動轉為大寫；同一編號不可重複登錄。"),
        ("hint", "若管理者於「資料庫設定 → 系統設定 → 罰單編號長度」設定了最少字數，"
                 "編號字數不足時按「新增」會被擋下並提示；設為 0 表示不限制。"),
        ]),
        ("warn", "預覽該列後可按文號<b>修改</b>或按 " + _btn_danger("✕")
                 + "<b>刪除</b>。刪除後該文號作廢不再用，原罰單編號仍可重新取號登錄。"),
        ("hint", _MODE_HINT),
        ("hint", "發文結算模式下，發文人員反灰不需填寫，"
                 "未發文罰單以橘字「未發文」顯示。"),
    ],
    ...
    5: [
        ...
        ("sec", "結算發文（有任一功能採發文結算模式或有未發文殘留資料）", [
            ("p", "只要管理者於「系統設定 → 陳報模式」將刑案陳報、一般陳報、敘獎登錄或罰單登錄的<b>任一功能</b>"
                  ...
                  + " 與未發文筆數。到了陳報日按此鈕："),
            ("ol", [
                "彈出視窗以單一表格列出所有<b>未發文</b>的刑案／一般／敘獎／罰單案件，包含先前切換模式前殘留的未發文公文。",
                "預設全部選取，可依種類分別檢視各項目。",
                "選<b>發文日期</b>（預設今天，可往前選）與<b>送文者</b>後確認，勾選的案件補上發文資訊，未勾選者維持未發文。",
                "結算發文只補上發文日期與發文人員，不會變更登錄日期。",
    ...
    6: [
        ("lead", "資料庫資料搜尋與瀏覽。<br>"
                 ...
                 + " " + _tab("☆ 敘獎登錄") + " " + _tab("🎫 罰單登錄")
                 + " 切換資料表。" + _simple),
        ("muted", "「公文快速登錄系統」只有<b>敘獎</b>與<b>罰單</b>兩個子頁。"),
        ("sec", "重新載入資料", [
    ...
    8: [
        ...
        ("sec", "維護清單（人員管理" + _simple + "／部門管理／案件類型）", [
            ("ul", [
                "<b>新增</b>：按 " + _btn("＋ 新增") + "。",
                "<b>修改</b>：選一列按 " + _btn("✎ 修改") + "，或直接<b>雙擊該列</b>（含停用／啟用、別名等）。",
                "<b>別名</b>：人員與案件類型可設多個別名（半形逗號分隔）。案類別名＝<b>通俗用語</b>，"
                "設定後陳報／修改／轉換視窗的「案件分類」可輸入通俗用語搜尋；人員別名用於歸檔檔名比對。",
        ...
        ("warn", "調整後務必按 " + _btn("💾 儲存排序")
                 + " 才會寫入。未存檔就切換頁面時會跳出提醒存檔。"),
        # ⚠️ 本區塊由上而下的排列必須與程式一致（tab_settings._SYSTEM_PANEL_ORDER），
        # 使用者是照著畫面找說明的。改面板順序時這裡要跟著改。
        ("sec", "系統設定", [
            ("p", "左側「系統設定」子頁集中七個區塊，各區塊修改後須按該區塊的「儲存」才會寫入。"),
            ...
            ("ul", [
                ...
                "網路位置若在<b>異地備份連線等待秒數</b>（預設 3 秒）內沒有回應，本次即略過異地備份、不拖慢開啟，主備份不受影響。",
            ...
            ("p", _note("陳報模式") + _simple + "（僅管理者）："),
            ("ul", [
                "<b>送文者輸入模式</b>（預設）：由送文者統一輸入案件，輸入時就填好陳報日期與發文人員。",
                "<b>發文結算模式</b>：承辦人自行陳報先取文號（暫不填發文資訊），"
                "送文者於陳報日到「簽收單列印」頁按「結算發文」補齊並印簽收表。"
                "發文結算模式影響刑案與一般陳報，以及敘獎登錄與罰單登錄；四項可<b>分別</b>設定。",
                "交辦單不提供此選項，因為必須由承辦人完成交辦事項後，該筆資料才算被認可。",
                "罰單簽收歸屬日一律依<b>發文日期</b>（實際發文或結算日期），與目前採送文者輸入模式或發文結算模式無關；切換模式不會改變歷史罰單的列印歸屬。",
                "儲存後立即生效；跨年度重置不清除。",
            ]),
            ("p", _note("罰單編號長度") + _simple + "（僅管理者）："),
            ("ul", [
                "設定罰單編號的最少字數，字數不足時登錄會被擋下並提示。",
                "設 0 為不限制；儲存後立即生效。",
            ]),
            ("p", _note("唯讀設定") + _simple + "（僅管理者）："),
            ("ul", [
                "可分別將<b>交辦單發文／交辦單收文／刑案陳報／一般陳報／敘獎登錄／罰單登錄</b>設為唯讀（多用於跨年度切換後，舊程式僅供查詢）。",
                "被停用的分頁頂端顯示紅色「唯讀模式」橫幅，"
                "<b>三種身分都不能新增、修改或刪除</b>，僅供瀏覽。",
                "儲存後立即生效。",
            ]),
            ("p", _note("閒置逾時") + _simple + "（僅管理者）："),
            ("ul", [
...
QUICKSTART = {
    0: ("將交辦單發文，一般使用者按下編號可修改承辦，按下 {X} 可從發文清單移除該列。",
        ["先選發文日期（預設今天）與發文人員。",
         "輸入文號後按 Enter 可加入預覽清單，可連續輸入。",
         "輸入完成後按「確認發文」即完成。"],
        "將發過的文(橘色日期)重新再發會覆蓋舊日期。"),
    ...
    3: ("登錄敘獎事由與敘獎人員，可連續輸入。",
        ["發文結算模式下，發文日期與發文人員反灰不需填寫。",
         "輸入敘獎事由，點候選人員名條或輸入姓名；"
         "多人以逗號分隔，名單外的姓名可直接輸入。",
         "按「確認送出」，系統自動記錄今天為登錄日期，並在本次預覽確認結果。"],
        None),
    4: ("登錄罰單，可使用掃描槍等工具掃描一維碼快速登錄。",
        ["發文結算模式下，發文人員反灰不需填寫。",
         "點候選人員名條或輸入姓名。",
         "輸入罰單編號後按 Enter 或「新增」，可連續輸入下一筆。"],
        ["罰單編號僅接受半形英文字母與數字，系統自動轉大寫；同一編號不可重複登錄。",
         "預覽該列後可按文號「修改」或按 {X}「刪除」。"
         "刪除後該文號作廢不再用，原罰單編號仍可重新取號登錄。",
         "本頁不設身分限制，一般使用者、歸檔管理、管理者皆可新增、修改、刪除。"]),
    5: ("依發文日期彙整當天所有發文，整理成簽收表。",
        ["選發文日期（預設今天）。",
         "按「產生預覽」顯示簽收表預覽。",
         "按「下載 PDF」存檔，或按「列印表單」送印（預設彩色＋長邊雙面）。"],
        "當結算發文模式啟用時，需先按下「結算發文」輸入陳報人員與日期，"
        "將各種公文類型(刑案／一般／敘獎／罰單)一次性發文，"
        "補齊發文資訊後自動產生新的簽收表。"),
    ...
    8: ("維護人員／部門／案類等基礎資料、系統設定與跨年度重置。本頁供管理者使用；歸檔管理僅能變更密碼與設定歸檔資料夾。",
        ["人員／部門／案類可按新增按鈕來新增，拉動或輸入新數字可改變排序，"
         "完成後按「儲存排序」才會寫入。",
         "「系統設定」子頁可修改目前的各種設定值及設定類型。",
         "「備份還原」子頁可把某份備份覆蓋回目前資料庫（還原前自動留底、完成後重啟）。"],
        ["歸檔管理員登錄後可額外使用檔案歸檔頁面；管理者可額外顯示操作紀錄。",
