...
HELP_PAGES = {
    ...
    4: [
        ...
        ("hint", "三種身分皆可使用本頁。若某筆在發文前已被刪除，系統會略過該筆並提示未更新筆數。"),
        ("hint", "頁面頂端若顯示紅色「唯讀模式」橫幅，表示管理者已於「資料庫設定 → 系統設定」"
    ...
    9: [
        ...
        ("sec", "系統設定", [
            ...
            ("ul", [
                ...
                "罰單登錄的模式切換同時改變<b>罰單簽收單的查詢基準</b>：自助取號模式依<b>發文日期</b>查詢，送文者輸入模式依<b>登錄日期</b>查詢；歷史單據的列印結果會隨目前模式而不同，切換前請留意。",
                "儲存後立即生效；跨年度重置不清除。",
            ]),
        ]),
        ("sec", "資源回收筒（僅管理者）", [
...
QUICKSTART = {
    ...
    6: ("依發文日期彙整當天所有發文，整理成簽收表。",
        ...
        "刑案陳報、一般陳報或罰單登錄任一功能設為自助取號模式時，本頁上方多出「結算發文」：單一表格列出所有未發文的刑案／一般／罰單案件，包含切換模式前殘留的未發文公文。三項皆為送文者輸入模式但仍有未發文殘留資料時，入口也會顯示，供補齊殘留案件；全部結算完成後即隱藏。可用類型標籤與關鍵字過濾、表頭全選（僅作用於顯示中列），選送文者後補齊發文資訊並自動產生簽收表；敘獎請至「敘獎發文」頁處理。"),
