...
HELP_PAGES = {
    ...
    3: [
        ...
        ("sec", "操作流程", [
            ("ol", [
                "選擇<b>發文日期</b>（預設今天）與同列的<b>發文人員</b>，輸入<b>敘獎事由</b>。",
        ...
        ("sec", "修改與刪除", [
            ("ul", [
                "輸入錯誤可按編號（藍色可點）開修改視窗。",
                "<b>發文日期</b>留空將被認定為未發文狀態；補填日期補發時須一併選<b>發文人員</b>，清空日期即退回未發文狀態。",
            ]),
        ]),
        ("hint", "常用人員會依歷來敘獎次數自動前移；名單外姓名可直接輸入，不必先加入人員清單。"),
        ("hint", "若管理者已於「系統設定 → 陳報模式」改為<b>自助取號模式</b>，登錄時的"
                 "「發文日期」與「發文人員」會反灰、視為未發文——你只是先取號，發文資訊由送文者"
                 "日後於「簽收單列印」頁按「結算發文」統一補齊；未發文案件在預覽與資料庫瀏覽以橘字「未發文」顯示。"),
    ],
    ...
    5: [
        ...
        ("sec", "結算發文（僅自助取號模式）", [
            ...
            ("ol", [
                "彈出視窗以單一表格列出所有<b>未發文</b>的刑案／一般／敘獎案件（預設全勾）。",
    ...
    8: [
        ...
        ("sec", "系統設定", [
            ...
            ("ul", [
                ...
                "<b>自助取號模式</b>：承辦人自行陳報先取文號（暫不填發文資訊），送文者於陳報日到「簽收單列印」頁按「結算發文」統一補齊並印簽收表；此模式同時涵蓋<b>敘獎登錄</b>，登錄時發文日期與發文人員反灰、視為未發文，一併於結算時補齊。",
                "儲存後立即生效；跨年度重置不清除。",
            ]),
        ]),
        ("sec", "資源回收筒（僅管理者）", [
...
QUICKSTART = {
    ...
    5: ("依發文日期彙整當天所有發文，整理成簽收表。",
        ...
        "自助取號模式下本頁上方多出「結算發文」：單一表格列出未發文的刑案／一般／敘獎，可用類型標籤與關鍵字過濾、表頭全選（僅作用於顯示中列），選送文者後補齊發文資訊並自動產生簽收表。"),
