...
HELP_PAGES = {
    ...
    2: [
        ...
        ("hint", "若管理者已於「系統設定 → 陳報模式」改為<b>自助取號模式</b>，陳報時的"
                 ...
                 "日後於「簽收單列印」頁按「結算發文」統一補齊；未發文案件在預覽與資料庫瀏覽以橘字「未發文」顯示。"),
    ...
    5: [
        ("lead", "登錄舉發罰單，一張罰單一位開立人員；分<b>送文者輸入模式</b>與<b>自助取號模式</b>兩種，由管理者於「系統設定 → 陳報模式」設定。"),
        ("sec", "操作流程", [
            ("ol", [
        ...
        ("hint", "若管理者已於「系統設定 → 陳報模式」啟用<b>自助取號模式</b>，發文者欄反灰不必填，"
    ...
    6: [
        ...
        ("sec", "結算發文（僅自助取號模式）", [
            ...
            ("ol", [
                "彈出視窗以單一表格列出所有<b>未發文</b>的刑案／一般／罰單案件（預設全勾）。敘獎請至「敘獎發文」頁處理。",
    ...
    9: [
        ...
        ("sec", "系統設定", [
            ...
            ("ul", [
                ...
                "<b>自助取號模式</b>：承辦人自行陳報先取文號（暫不填發文資訊），送文者於陳報日到「簽收單列印」頁按「結算發文」統一補齊並印簽收表。自助取號模式影響刑案與一般陳報，以及<b>罰單登錄</b>（發文者欄反灰、發文資訊改由結算補齊）；<b>敘獎登錄與敘獎發文不受陳報模式影響</b>，請分別於對應頁面操作。",
                "陳報模式切換同時改變<b>罰單簽收單的查詢基準</b>：自助取號模式依<b>發文日期</b>查詢，送文者輸入模式依<b>登錄日期</b>查詢；歷史單據的列印結果會隨目前模式而不同，切換前請留意。",
                "儲存後立即生效；跨年度重置不清除。",
            ]),
        ]),
        ("sec", "資源回收筒（僅管理者）", [
...
QUICKSTART = {
    ...
    5: ("登錄舉發罰單，一張罰單一位開立人員；分送文者輸入模式與自助取號模式。",
    ...
    6: ("依發文日期彙整當天所有發文，整理成簽收表。",
        ...
        "自助取號模式下本頁上方多出「結算發文」：單一表格列出未發文的刑案／一般／罰單案件，可用類型標籤與關鍵字過濾、表頭全選（僅作用於顯示中列），選送文者後補齊發文資訊並自動產生簽收表；敘獎請至「敘獎發文」頁處理。"),
    ...
    9: ("維護人員／部門／案類等基礎資料、系統設定與跨年度重置。本頁供管理者使用；歸檔管理僅能變更密碼與設定歸檔資料夾。",
        ["新增或修改清單項目（雙擊整列可修改，含停用／啟用、別名）。",
         ...
         "「系統設定」子頁設定歸檔資料夾、簽收表標題、閒置逾時、七種流程的唯讀設定、自動備份（第二備份位置）與陳報模式。",
