...
#   0 交辦單發文 / 1 交辦單收文 / 2 公文陳報 / 3 敘獎登錄 / 4 敘獎發文 / 5 罰單登錄
#   6 簽收單列印 / 7 資料庫瀏覽 / 8 檔案歸檔 / 9 資料庫設定 / 10 操作紀錄
...
HELP_TITLES = {
    ...
    4: "敘獎發文", 5: "罰單登錄", 6: "簽收單列印", 7: "資料庫瀏覽",
    8: "檔案歸檔", 9: "資料庫設定", 10: "操作紀錄",
}
...
HELP_PAGES = {
    ...
    2: [
        ...
        ("hint", "管理者可在「系統設定 → 陳報模式」分別設定刑案陳報與一般陳報。若目前頁籤設為"
                 "<b>自助取號模式</b>，「陳報日期」與「發文人員」會反灰不必填；切換上方頁籤時，"
    ...
    3: [
        ...
        ("muted", "本頁在「公文快速登錄系統」也有，操作方式相同、共用同一份資料；發文請回本系統的「敘獎發文」頁處理。"),
        ("sec", "操作流程", [
            ("ol", [
                "輸入<b>敘獎事由</b>。",
        ...
        ("hint", "新登錄資料一律為未發文；請由「敘獎發文」頁輸入編號，批次填入發文日期與發文人員。"),
    ],

    4: [
        ("lead", "輸入所有敘獎編號後統一發文，並在資料庫加上發文日期與發文人員。"),
        ("sec", "操作流程", [
            ("ol", [
                "先選擇<b>發文日期</b>（預設今天）與<b>發文人員</b>。",
                "在文號輸入框輸入編號後按 " + _key("Enter") + "，或按「輸入」；可連續輸入多筆。",
                "確認清單後按 " + _btn_primary("確認發文") + "，確認框會顯示日期、人員與筆數，確認後一次寫入。",
            ]),
        ]),
        ("sec", "清單狀態", [
            ("ul", [
                "發文日期空白代表<b>尚未發文</b>。",
                "發文日期顯示橘字代表該筆<b>已發文</b>；本次確認後會覆蓋原發文日期與發文人員。",
                "輸入不存在的編號時，系統會提示找不到編號；輸入已刪除的編號時，系統會提示已被刪除。",
                "輸入錯誤可按該列最左的 " + _btn_danger("✕") + " 移出待發清單，不會刪除敘獎資料。",
            ]),
        ]),
        ("hint", "三種身分皆可使用本頁。若某筆在確認發文前已被其他電腦刪除或搶先發文，系統會略過該筆，"
                 "並於結果訊息分別列出已刪除與已被他人發文的筆數；訊息關閉後清單會重新載入，只留下仍未發文的編號。"),
        ("hint", "頁面頂端若顯示紅色「唯讀模式」橫幅，表示管理者已於「資料庫設定 → 系統設定」"
                 "停用敘獎發文（多用於跨年度切換後），一般使用者僅供瀏覽；此開關與敘獎登錄分開。"),
    ],
    ...
    5: [
        ...
        ("sec", "操作流程", [
            ("ol", [
                "送文者輸入模式須先選<b>發文人員</b>；自助取號模式此欄反灰不必填。",
        ...
        ("hint", "本頁不設身分限制，一般使用者、歸檔管理、管理者皆可新增、修改、刪除罰單資料（含已發文資料）；"
                 "「資料庫瀏覽」頁的罰單資料則<b>僅管理者</b>可修改。"),
        ("hint", "若管理者將<b>罰單登錄</b>設為<b>自助取號模式</b>，發文人員欄反灰不必填，"
    ...
    6: [
        ("lead", "選擇<b>發文日期</b>並列印簽收表，自助取號模式時可結算發文。"),
        ("sec", "操作流程", [
            ("ol", [
                "選<b>發文日期</b>（預設今天）。",
        ...
        ("sec", "結算發文（有任一功能採自助取號模式或有未發文殘留資料）", [
            ("p", "只要管理者於「系統設定 → 陳報模式」將刑案陳報、一般陳報或罰單登錄的<b>任一功能</b>"
                  "設為<b>自助取號模式</b>，本頁上方就會多出 " + _key("結算發文")
                  + " 與未發文筆數。到了陳報日按此鈕："),
            ("ol", [
                "彈出視窗以單一表格列出所有<b>未發文</b>的刑案／一般／罰單案件（預設全勾），包含先前切換模式前殘留的未發文公文。敘獎請至「敘獎發文」頁處理。",
            ...
            ("note", "勾選中含刑案、一般或敘獎任一類時，送文者為必填。"),
        ]),
        ("hint", "三項皆為送文者輸入模式但仍有未發文殘留資料時，本頁仍顯示「結算發文」入口，"
    ...
    9: [
        ...
        ("sec", "系統設定", [
            ...
            ("p", _note("唯讀設定") + "（僅管理者）："),
            ("ul", [
                "可分別將<b>交辦單發文／交辦單收文／刑案陳報／一般陳報／罰單登錄／敘獎登錄／敘獎發文</b>設為唯讀（多用於跨年度切換後，舊程式僅供查詢）。",
                "敘獎登錄與敘獎發文為兩個獨立開關，可只停用其中一頁。",
            ...
            ("ul", [
                ...
                "<b>自助取號模式</b>：承辦人自行陳報先取文號（暫不填發文資訊），送文者於陳報日到「簽收單列印」頁按「結算發文」統一補齊並印簽收表。自助取號模式影響刑案與一般陳報，以及罰單登錄；三項可<b>分別</b>設定模式。罰單設為自助取號時，發文人員欄反灰、發文資訊改由結算補齊。",
                "敘獎登錄與敘獎發文不受陳報模式影響。交辦單與敘獎不提供此選項，因為必須由承辦人完成交辦事項、由敘獎人確認敘獎內容後，該筆資料才算被認可。",
                "罰單簽收歸屬日一律依<b>發文日期</b>（實際發文或結算日期），與目前採送文者輸入模式或自助取號模式無關；切換模式不會改變歷史罰單的列印歸屬。",
                "儲存後立即生效；跨年度重置不清除。",
            ]),
        ]),
        ("sec", "資源回收筒（僅管理者）", [
...
HELP_TIPS = {
    ...
    4: {
        "btn_reward_input": "輸入敘獎編號後加入待發清單",
        "btn_reward_issue": "依所選發文日期與人員，將清單一次全部發文",
        "btn_reward_issue_clear": "清空目前待發清單，不會刪除敘獎資料",
    },
    5: {
        "ticket_add":            "輸入開立人員與罰單編號後加入本次登錄",
        "ticket_clear_issuer":   "清空開立人員欄，發文人員與罰單編號不動",
        "ticket_candidates_list": "點選常用人員直接取代目前開立人員",
    },
    6: {
...
QUICKSTART = {
    ...
    2: ("登錄刑案與業務案件，由系統自動配發文號。按下 {X} 可刪除該筆，該文號將作廢。",
        ["上方頁籤切換「刑案／一般」表單；兩種陳報可分別設定送文者輸入或自助取號模式。",
         "目前頁籤若設為自助取號模式，陳報日期與發文人員反灰不必填；切換頁籤時兩欄狀態會跟著改變。",
         "填妥對應欄位。",
         "按「確認陳報」取得文號。"],
    ...
    3: ("登錄敘獎事由與敘獎人員，可連續輸入。",
        ...
        "新登錄資料一律為未發文；常用人員會依歷來敘獎次數自動前移。"),
    4: ("輸入敘獎編號建立待發清單，再批次填入發文日期與發文人員。按下 {X} 只會移出清單，不會刪除資料。",
        ["先選發文日期（預設今天）與發文人員。",
         "輸入敘獎編號後按 Enter 或「輸入」，可連續輸入多筆。",
         "按「確認發文」，確認筆數後一次寫入。"],
        ["發文日期顯示橘字代表該筆已發文，本次將覆蓋原發文資訊；若發文前已被刪除，系統會略過並提示。",
         "紅色「唯讀模式」橫幅表示一般使用者只能瀏覽；敘獎登錄與敘獎發文可由管理者分開停用。"]),
    5: ("登錄舉發罰單，一張罰單一位開立人員；可單獨設定送文者輸入模式或自助取號模式。",
        ["送文者輸入模式先選發文人員；自助取號模式此欄反灰不必填。",
    ...
    6: ("依發文日期彙整當天所有發文，整理成簽收表。",
        ["選發文日期（預設今天）。",
         "按「產生預覽」顯示簽收表預覽。",
         "按「下載 PDF」存檔，或按「列印表單」送印（預設彩色＋長邊雙面）。"],
        "刑案陳報、一般陳報或罰單登錄任一功能設為自助取號模式時，本頁上方多出「結算發文」：單一表格列出所有未發文的刑案／一般／罰單案件，包含切換模式前殘留的未發文公文。三項皆為送文者輸入模式但仍有未發文殘留資料時，入口也會顯示，供補齊殘留案件；全部結算完成後即隱藏。可用類型標籤與關鍵字過濾、表頭全選（僅作用於顯示中列），選發文日期（預設今天、可回填）與送文者後補齊發文資訊並自動產生簽收表；敘獎請至「敘獎發文」頁處理。"),
    7: ("查詢資料。上方分交辦單／刑案／一般／罰單／敘獎五個子頁。",
        ["搜尋框輸入關鍵字，邊打邊自動比對。",
         "用範圍下拉可限定只比某一欄。",
         "按「完整」顯示全部欄位（預設精簡只顯示重要欄位）。"],
        ["交辦單頁左下篩選鈕可只看逾期未回；主旨欄左側 PDF 圖示可直接開檔。",
         "建錯類別（刑案／一般）可點「編號」開修改視窗、按底部「轉換類別」鈕一鍵互轉（僅管理者／歸檔管理，原號作廢不可復原）。"]),
    8: ("把電子檔 PDF 歸檔並改名。本頁供管理者與歸檔管理使用。",
        ["左側清單點選一筆待歸檔公文。",
         "中間候選 PDF 依命中字數排序，確認正確檔案。",
         "按「歸檔預覽」解析檔名，逐格核對日期／主旨／承辦人。",
         "按「檔案歸檔」完成。"],
        "開始前先確認歸檔資料夾已設定；只有紙本按「只歸紙本」。"),
    9: ("維護人員／部門／案類等基礎資料、系統設定與跨年度重置。本頁供管理者使用；歸檔管理僅能變更密碼與設定歸檔資料夾。",
        ["新增或修改清單項目（雙擊整列可修改，含停用／啟用、別名）。",
         "拖拉整列（或單擊序號欄輸入數字）調整顯示順序，務必按「儲存排序」才會寫入。",
         "「系統設定」子頁設定歸檔資料夾、簽收表標題、閒置逾時、七種流程的唯讀設定、自動備份（第二備份位置），並分別設定刑案陳報、一般陳報與罰單登錄的陳報模式。",
         "「備份還原」子頁可把某份備份覆蓋回目前資料庫（還原前自動留底、完成後重啟）。"],
        ["自助取號模式影響刑案與一般陳報，以及罰單登錄；三項可分別設定。敘獎登錄與敘獎發文不受陳報模式影響。",
         "交辦單與敘獎不提供陳報模式選項，因為必須由承辦人完成交辦事項、由敘獎人確認敘獎內容後，該筆資料才算被認可。",
